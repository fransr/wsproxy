...
${log.infoHighlight(`--quiet / -q`)}                            no output for messages
${log.infoHighlight(`--proxy / -x <host>`)}                            use proxy for api-calls, example: -x http://localhost:8000
...
`)
    ...
    argument_parse: async (args) => {
        ...
        if (!settings.runTests) {
            ...
            if (settings.proxy && !settings.proxy.match(/^https?:\/\//)) log.die('proxy (-x) not valid, url to use as proxy');
