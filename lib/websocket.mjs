...
export const websocket = {
    ...
    addHeaders: async (headers) => {
        ...
        for (const header of headerData) {
            ...
            if(!headerKey.trim().length || headerKey.toLowerCase() == 'content-type' || headerKey.toLowerCase() == 'accept-encoding' || headerKey.toLowerCase() == 'content-length'){
                continue;
            }
    ...
    addConnection: async (idx) => {
        ...
        websocket.initConnection(wsclient, idx);
        log.action(`${idx} connecting to socket`);
        const headers = await websocket.addHeaders([]);
        const origin = utils.getHeader('origin');
        wsclient.connect(
